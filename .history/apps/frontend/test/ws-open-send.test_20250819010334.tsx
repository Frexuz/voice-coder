...
class ControlledWS {
  static last: ControlledWS | null = null;
  readyState = 1; // OPEN
...
test("WS open: sending text sends over WS and shows reply", async () => {
  await withWS(ControlledWS, async () => {
    ...
    const ws = ControlledWS.last!;
    expect(ws.sends.length).toBe(1);

    // Server replies
    ws.onmessage?.({ data: JSON.stringify({ type: "reply", id: "1", text: "ok" }) } as any);
